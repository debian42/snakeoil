// all credits go to:
// https://www.codeproject.com/Articles/8843/Win32-file-name-iteration-STL-way
#ifndef FILEITER_H
#include <windows.h>
#include <iterator>
#include <string>
...
public:
	 ...
	 win32_file_iterator(std::string const& strfilter, bool bFullPath = false,
	...
	win32_file_iterator(win32_file_iterator const& rhs) {
		_handle = rhs._handle;
		_handle.incRef();
		_flag = rhs._flag;
	...
	bool operator==(win32_file_iterator const& rhs) const {
...
private:
	...
	void _chkvalid(WIN32_FIND_DATA & wfd) {
		if (wfd.dwFileAttributes & _flag) {
			_getval(wfd);
		}
		else {
			_findnext();
	...
	void _getval(WIN32_FIND_DATA const& wfd) {
		if (_bFullPath)
